...
FOR UPDATE`, requestID).Scan(&status, &requestedRevision)
	...
	suggestions, err := lockOpenReviewSuggestions(ctx, tx, requestID, suggestionIDs)
	if err != nil {
		return domain.Page{}, err
	}
...
// lockOpenReviewSuggestions locks selected unapplied suggestions and returns their persisted source edits.
