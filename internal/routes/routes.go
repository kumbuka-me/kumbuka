...
func (r routeRegistrar) addPageRoutes() {
	...
	r.mux.Handle("POST /pages/approval/update/{id}/{slug...}", browserAuthn(editorAuthz(r.pageEditAuthz(handler.UpdatePageReview(config.Pages, config.Logger)))))
	r.mux.Handle("POST /pages/approval/cancel/{id}/{slug...}", browserAuthn(editorAuthz(r.pageEditAuthz(handler.CancelPageReview(config.Pages, config.Logger)))))
	r.mux.Handle("POST /pages/approval/decide/{id}/{slug...}", browserAuthn(editorAuthz(r.pageViewAuthz(handler.DecidePageReview(config.Pages, config.Logger)))))
	...
	r.mux.Handle("POST /page-comments/resolve/{id}/{slug...}", browserAuthn(editorAuthz(r.pageEditAuthz(handler.ResolvePageComment(config.Pages, config.Views)))))
...
func (r routeRegistrar) addAPIRoutes() {
	...
	r.mux.Handle("GET /api/page-presence/{slug...}", apiAuthn(r.pageViewAuthz(handler.PageEditors(config.Pages, config.Logger))))
	r.mux.Handle("PUT /api/page-presence/{slug...}", apiAuthn(editorAuthz(r.pageEditAuthz(handler.TouchPageEditor(config.Pages, config.Logger)))))
	r.mux.Handle("DELETE /api/page-presence/{slug...}", apiAuthn(editorAuthz(r.pageEditAuthz(handler.LeavePageEditor(config.Pages, config.Logger)))))
