...
func SaveAdminEditorToolbar(settingsUseCases settingsService, manager *plugin.Manager, views *webview.Views) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		settings.EditorToolbarOverrides, err = editorToolbarOverridesFromForm(r, manager, settings.EditorToolbarOverrides)
		if err != nil {
		...
		if err := settingsUseCases.SaveApplicationSettings(r.Context(), settings, currentUser(r).ID); err != nil {
			writeAdminProblem(views.Logger(), w, err, "Editor toolbar")
			return
		}
		http.Redirect(w, r, "/admin/configuration#editor-toolbar", http.StatusSeeOther)
	}
}
...
func editorToolbarOverridesFromForm(r *http.Request, manager *plugin.Manager, existing []domain.EditorToolbarOverride) ([]domain.EditorToolbarOverride, error) {
	...
	for index, id := range ids {
		...
		override := domain.EditorToolbarOverride{ID: id, Group: group, Hidden: r.FormValue("toolbar_visible_"+id) != "on", Order: order}
