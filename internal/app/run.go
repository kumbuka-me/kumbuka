...
func runtimeInfo(cfg flags.Config, secretCipher *secrets.Cipher) webview.RuntimeInfo {
	...
	return webview.RuntimeInfo{
		...
		ManagedConfiguration:               managedConfiguration(cfg, secretCipher),
	}
}

// managedConfiguration returns deployment-owned configuration using only administrator-safe values.
func managedConfiguration(cfg flags.Config, secretCipher *secrets.Cipher) []webview.ManagedConfigurationGroup {
	return []webview.ManagedConfigurationGroup{
		...
		},
		{
			...
			Items: []webview.ManagedConfigurationItem{
				...
				managedConfigurationItem(cfg, "Application encryption key", "encryption-key", configuredLabel(secretCipher.Configured())),
			},
		},
		{
			Name: "Logging",
